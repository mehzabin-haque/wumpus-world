...
export default function Cheat({ piece, row, col, agentPosition }: Props) {
  ...
  return (
    <div data-row={row} data-col={col}
      className={`square h-12 w-12 text-6xl flex items-center justify-center outline outline-[#171c28] bg-[#A8F2FE]
        }`}
    >
      ...
      {(agentPosition.row !== row || agentPosition.column !== col) && piece === 'SG' && <Image src={`/images/gold.png`} alt={''} width={64} height={64} />}
      {(agentPosition.row !== row || agentPosition.column !== col) && piece === 'breeze' && <Image src={`/images/wind.png`} alt={''} width={36} height={64} />}
      {(agentPosition.row !== row || agentPosition.column !== col) && piece === 'stench' && <Image src={`/images/waste.png`} alt={''} width={36} height={64} />}
      {(agentPosition.row !== row || agentPosition.column !== col) && piece === 'stenchbreeze' && <Image src={`/images/odor.png`} alt={''} width={36} height={64} />}
    </div>
